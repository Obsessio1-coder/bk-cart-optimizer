...
def test_basic():
    ...
    check("returns tuple of 9", len(r) == 9)
    best_total, savings, best_state, menu_total, menu_prices, effective_prices, mono_plan, dish_idx, order = r
...
def test_single_whoper():
    ...
    check("single item returns tuple", len(r) == 9)
    best_total, savings, best_state, menu_total, _, _, _, _, _ = r
    check("best_total > 0", best_total > 0, str(best_total))
    check("savings >= 0", savings >= 0, str(savings))
...
def test_fuzzy_match():
    ...
    check("fuzzy returns tuple", len(r) == 9)
    _, _, _, _, _, _, _, _, order = r
...
def test_price_kopecks_integrity():
    ...
    check("price test returns tuple", len(r) == 9)
    best_total, savings, best_state, menu_total, _, _, _, _, _ = r
...
def test_remaining():
    ...
    check("remaining test returns tuple", len(r) == 9)
    _, _, best_state, _, _, _, _, _, _ = r
...
def test_different_store():
    ...
    check("store 1003 returns tuple", len(r) == 9)
    _, _, _, _, _, _, _, _, order = r
...
def test_mono_coupon_used():
    ...
    check("mono coupon returns tuple", len(r) == 9)
    _, _, best_state, _, _, _, mono_plan, _, _ = r
    if mono_plan:
...
def test_all_mono_possible():
    ...
    check("mono test returns tuple", len(r) == 9)
    best_total, savings, _, menu_total, _, effective_prices, mono_plan, _, order = r
    if mono_plan:
...
def test_generate_alt_carts():
    ...
    check("alt test returns tuple", len(r) == 9)
    _, _, best_state, _, _, _, _, _, _ = r
    best_costs = best_state[4]
...
def test_cola_expansion():
    ...
    check("cola expansion returns tuple", len(r) == 9)
    _, _, best_state, _, _, _, _, _, _ = r
    best_costs = best_state[4]
    check("best_costs has saving_tip", "saving_tip" in best_costs)
...
def test_nuggets_expansion():
    r = optimize(["Воппер", "Наггетсы (9 шт.)", "Кола"], restaurant_id="1002", mode="offline")
    check("nuggets expansion returns tuple", len(r) == 9)
    _, _, best_state, _, _, _, _, _, _ = r
    best_costs = best_state[4]
    check("best_costs has saving_tip", "saving_tip" in best_costs)
...
def test_fries_expansion():
    ...
    check("fries expansion returns tuple", len(r) == 9)
    best_total, savings, best_state, menu_total, _, _, _, _, _ = r
    best_costs = best_state[4]
    check("savings >= 0", savings >= 0, str(savings))
...
def test_project_example():
    r = optimize(["Воппер", "Кинг Фри стандартный", "Кола"], restaurant_id="1002", mode="offline")
    check("example returns tuple", len(r) == 9)
    best_total, savings, _, menu_total, _, _, _, _, _ = r
...
def test_double_mono_coupon():
    ...
    check("double mono returns tuple", len(r) == 9)
    best_total, savings, best_state, menu_total, _, _, mono_plan, _, order = r
...
def test_multi_slot_sauce_coupon():
    ...
    check("multi-slot test returns tuple", len(r) == 9)
    best_total, _, best_state, menu_total, _, _, _, _, _ = r
    bc = best_state[4]
    check("best_total < menu_total", best_total < menu_total, f"{best_total} < {menu_total}")
    check("has_multi_slot_savings", best_total > 0, str(best_total))
    # 2 sauces @ 99.99 via multi-slot + Чикенбургер @ 99.99 mono + Кола @ 99.99 mono = 299.97
    check("total ~ 299.97", abs(best_total - 299.97) < 0.01, str(best_total))

def test_multi_slot_3_sauces():
    r = optimize(["Соус Чесночный", "Соус Чесночный", "Соус Чесночный", "Кола"], restaurant_id="1002", mode="offline")
    check("multi-slot 3 returns tuple", len(r) == 9)
    best_total, _, best_state, menu_total, _, _, _, _, _ = r
    check("best_total < menu_total", best_total < menu_total, f"{best_total} < {menu_total}")
    # 3 sauces @ 119.99 via 3-slot coupon + Кола @ 99.99 mono = 219.98
    check("total ~ 219.98", abs(best_total - 219.98) < 0.01, str(best_total))
...
def test_multi_slot_excess_4_sauces():
    ...
    check("multi-slot 4 returns tuple", len(r) == 9)
    best_total, _, _, menu_total, _, _, _, _, _ = r
    check("best_total < menu_total", best_total < menu_total, f"{best_total} < {menu_total}")
    # 3-slot coupon @ 119.99 + 1 remaining sauce @ 59.99 = 179.98
    check("total ~ 179.98", abs(best_total - 179.98) < 0.01, str(best_total))

def test_multi_slot_mixed():
    r = optimize(["Соус Чесночный", "Соус Чесночный", "Воппер", "Кола"], restaurant_id="1002", mode="offline")
    check("multi-slot mixed returns tuple", len(r) == 9)
    best_total, _, _, menu_total, _, _, _, _, _ = r
    check("best_total < menu_total", best_total < menu_total, f"{best_total} < {menu_total}")
